...
def main() -> int:
    ...
    head = ["# The catalog", "",
            f"All **{len(entries)}** ready-to-use prompts, each paired with the image and "
            "seed from its generation.", "",
            "Every image is the original model output, so you can compare the prompt "
            "and result directly.",
            "The seeds reproduce on fal's hosted endpoint and not in a local graph; "
            "[REPRODUCING.md](../REPRODUCING.md) explains why.", "",
